...
def difficulty_window():
    ...
    a = Label(window,image=logo).grid(row = 0,column = 0,columnspan = 2 ,sticky = N+E+S+W)
    window.title('Sudoku')
    window.geometry('400x400+300+300')
    text = Label(window, text = '                          Select Difficulty ').grid(row = 1, column = 0)
    but_1=Button(window, text="Easy",command  = lambda window = window : set_difficulty(window,5)).grid(row = 3,column = 0,columnspan=2,rowspan = 2 ,sticky=N+S+E+W,)
    but_2=Button(window, text="Hard",command=lambda window = window : set_difficulty(window,4)).grid(row = 5,column = 0 ,columnspan=2,rowspan = 2 ,sticky=N+E+S+W)
    but_3=Button(window, text="Very Hard",command=lambda window = window : set_difficulty(window,3)).grid(row = 7,column = 0,columnspan=2 ,rowspan = 2 ,sticky=N+E+W+S )
    but_4=Button(window, text="Insane",command=lambda window = window : set_difficulty(window,2)).grid(row = 9,column = 0,columnspan=2 ,rowspan = 2 ,sticky=N+E+S+W)
    window.mainloop()
difficulty_window()
makegame()
def mainwindow():
    ...
    def DisplayVictory():
        ...
        disp = "Congratulations!  YOU WON in - " + str(round((start-now)/60,2)) + "minutes"
    ...
    def callback(entryText,a,b):
        ...
        elif val[0].isalpha() or len(val)>1:
             entryText.set( "" )
        elif val != str(s[a][b]):
    ...
    for i in range(11):
        ...
        for j in range(11):
            ...
            label = (a,b)
            entryText = StringVar()
            d[label] = Entry(window,fg = 'green', width=2,justify=CENTER, textvariable=entryText  )
            d[label].grid(row=i, column=j, sticky=N+E+S+W, pady = 1.5, padx = 1.5)
            if sudoku[a][b] == 0:
                entryText.set( "" )
                entryText.trace("w", lambda name, index, mode, entryText=entryText,a=a,b=b: callback(entryText,a,b))
            else:
